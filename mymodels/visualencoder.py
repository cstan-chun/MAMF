...
from mymodels.efficientface import InvertedResidual, LocalFeatureExtractor
from mymodels.modulator import Modulator
...
def TemporalUpsampleConv(input_dim, output_dim, scale_factor):
    return nn.Sequential(nn.ConvTranspose1d(input_dim, output_dim, kernel_size=scale_factor, stride=scale_factor),
                         nn.BatchNorm1d(output_dim),
                         nn.ReLU(inplace=True))


class EfficientFaceTemporal(nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, im_per_sample=15, feature_dim=128):
        ...
        self.conv1d_transpose = TemporalUpsampleConv(output_channels, output_channels, scale_factor=2)

        self.conv1d_0 = conv1d_block(output_channels, feature_dim // 2)
        self.conv1d_1 = conv1d_block(feature_dim // 2, feature_dim)
